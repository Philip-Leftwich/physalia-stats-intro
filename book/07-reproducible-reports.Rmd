...
Having added some in-line code, try re-knitting your .Rmd file, what is the output?
...
```{block, type = "warning"}
You might want start using the `here()` from now on to read in and export data from scripts. Make sure you are consistent in whether you use `here()` heuristic file paths or relative file paths across all files in a project - otherwise you might encounter errors. 
...
```{r, echo = FALSE}
penguins <- read_csv("files/penguins_raw.csv")
```
